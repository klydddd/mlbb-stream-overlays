...
| **Draft Control**   | `SWITCH TEAMS` Button | `#switch` | `js/control.js` | 🟡 **Buggy** | Swaps picks and names locally and broadcasts them. **Bugs:** (1) Does not swap camera links. (2) Does not write swapped player names back to `localStorage`, causing them to revert on reload. |
