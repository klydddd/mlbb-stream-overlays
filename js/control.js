...
for (let i = 1; i <= 10; i++) {
  const input = document.getElementById(`pick-${i}`);
  const savedPick = localStorage.getItem(`pick-${i}`);
  if (input && savedPick) {
    input.value = savedPick;

    // Send saved value to INDEX.html
    channel.postMessage({ [`pick-${i}`]: savedPick });
  }
}
...
const heroes = [
    ...
    { name: 'Minotaur', img: 'Assets/HeroPick/minotour.png', voice: 'Assets/Voicelines/minotour.ogg' },
...
if (resetBtn) {
  resetBtn.addEventListener("click", () => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      localStorage.removeItem(`heroPick-${i}`);
      localStorage.removeItem(`banPick-${i}`);
    }
...
channel.onmessage = (event) => {
  ...
  if (data.type === "switch") {
    ...
    if (blue) blue.textContent = data.blueTeamName || "Blue Team";
    if (red) red.textContent = data.redTeamName || "Red Team";
  ...
  if (data.type === "reset") {
    ...
    resetText("blue-team-name", "Blue Team");
    resetText("red-team-name", "Red Team");
  ...
  if (data.blueTeamName !== undefined) {
    ...
    if (blueDiv) blueDiv.textContent = data.blueTeamName || "Blue Team";
  }

  // 🔴 Single Red Team update
  if (data.redTeamName !== undefined) {
    const redDiv = document.getElementById("red-team-name");
    if (redDiv) redDiv.textContent = data.redTeamName || "Red Team";
  }
