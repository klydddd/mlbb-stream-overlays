...
const channel = new BroadcastChannel("hero_channel");

if (page === "controller") {
 const inputs = [...Array(10)].map((_, i) => document.getElementById(`pick-${i + 1}`));
...
function updateHeroPick(index, heroName) {
    ...
    if (hero && container) {
        container.innerHTML = `
            ...
            <audio autoplay>
                <source src="${hero.voice}" type="audio/ogg">
            </audio>
        `;
    } else if (container) {
        container.innerHTML = ''; // clear if name is invalid
    }
}

window.addEventListener('storage', (e) => {
    if (e.key && e.key.startsWith('heroPick-')) {
        const index = parseInt(e.key.split('-')[1]) - 1;
        updateHeroPick(index, e.newValue);
    }
});
